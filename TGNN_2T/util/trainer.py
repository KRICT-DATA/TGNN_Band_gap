...
def train(model, optimizer, data_loader, criterion):
    ...
    for batch in data_loader:
        pred = model(batch[0], batch[1], batch[3])
        loss = criterion(batch[2], pred)
...
def test(model, data_loader, criterion):
    ...
    with torch.no_grad():
        for batch in data_loader:
            pred = model(batch[0], batch[1], batch[3])
            test_loss += criterion(batch[2], pred).detach().item()
            test_rmse = rmse(batch[2], pred).detach().item()

            list_idxs.append(batch[4].cpu().numpy().reshape(-1, 1))
            list_targets.append(batch[2].cpu().numpy().reshape(-1, 1))
...
def collate(batch):
    ...
    return pairs, idx_pairs, targets, ref_feats, idxs
